...
namespace pinktrombone::noise
{
    ...
    {
    public:
        Grad() : Grad(0.f, 0.f, 0.f) {}
        Grad(double x, double y, double z) : x_{ x }, y_{ y }, z_{ z } {}
    ...
    std::array<int, 256> p = { 151,160,137,91,90,15,
    ...
    // This isn't a very good seeding double, but it works ok. It supports 2^16
    ...
    double F2 = 0.5f * (std::sqrt(3.f) - 1.f);
    double G2 = (3.f - std::sqrt(3.f)) / 6.f;
    ...
    double simplex2(double xin, double yin)
    {
        ...
        i &= 255;
        j &= 255;
        auto& gi0 = gradP[i + perm[j]];
        auto& gi1 = gradP[i + i1 + perm[j + j1]];
        auto& gi2 = gradP[i + 1 + perm[j + 1]];
