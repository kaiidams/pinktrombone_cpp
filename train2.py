...
class PinkTromboneModel(pl.LightningModule):
    def __init__(
        ...
        self.decoder = ErrorEstimator(self.articulator.input_dim, n_mfcc, hidden_dim)
    ...
    def decode(self, control: torch.Tensor) -> torch.Tensor:
        return self.decoder(control)
    ...
    def training_step(self, batch, batch_idx):
        ...
        g_loss = torch.mean(self.criterion(estimates, targets))
        ...
        d_loss = self.criterion(estimates, outputs)
        ...
        f_loss = self.criterion(estimates, outputs)
...
def train():
    model = PinkTromboneModel(
        batch_size=2 if small else 32,
        sample_rate=22_050,
        segment_length=640 * 256,
        padding='same',
        dataset='librispeech-dev' if small else 'librispeech')
    trainer = pl.Trainer(
        max_epochs=10000,
        log_every_n_steps=2,
        # precision='16-mixed',
